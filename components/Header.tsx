...
export default function Header() {
  ...
  return (
    <header className="site-header sticky top-0 z-50 shrink-0 border-b backdrop-blur-2xl">
      ...
      <div className="header-primary grid w-full grid-cols-[1fr_auto] items-center gap-x-4 px-4 py-3 md:px-6 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
        <Link href={`/${locale}`} className="header-brand flex shrink-0 items-center gap-3 whitespace-nowrap">
        ...
        <nav className="hidden gap-1 lg:flex lg:justify-center" aria-label="Primary navigation">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
      ...
      <nav className="mobile-nav flex gap-2 overflow-x-auto px-4 pb-3 lg:hidden" aria-label="Primary navigation">
        {links.map((link) => (
          <Link
            key={link.href}
            href={link.href}
